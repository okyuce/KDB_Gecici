...
public class BasvuruDosyaIndirCommand : IRequest<ResultModel<BasvuruDosyaIndirCommandResponseModel>>
{
        public string BasvuruDosyaId { get; set; } = string.Empty;
		public string TcKimlikNo { get; set; } = string.Empty;

	public class BasvuruDosyaIndirCommandHandle : IRequestHandler<BasvuruDosyaIndirCommand, ResultModel<BasvuruDosyaIndirCommandResponseModel>>
    {
        private readonly IBasvuruDosyaRepository _basvuruDosyaRepository;
        private readonly IConfiguration _configuration;

        public BasvuruDosyaIndirCommandHandle(IBasvuruDosyaRepository basvuruDosyaRepository, IConfiguration configuration)
        {
            _basvuruDosyaRepository = basvuruDosyaRepository;
            _configuration = configuration;
        }

        public async Task<ResultModel<BasvuruDosyaIndirCommandResponseModel>> Handle(BasvuruDosyaIndirCommand request, CancellationToken cancellationToken)
        {
            var result = new ResultModel<BasvuruDosyaIndirCommandResponseModel>();
            ...
            try
            {
                var fileGuid = Guid.Parse(request.BasvuruDosyaId);

                var file = _basvuruDosyaRepository
                        .GetWhere(x => x.BasvuruDosyaGuid == fileGuid
                            && x.Basvuru.TcKimlikNo == request.TcKimlikNo.Trim()
                            && x.AktifMi == true
                            && x.SilindiMi == false, true, i => i.Basvuru
                        )
                            .FirstOrDefault();

                if (file == null)
                {
                    result.Exception(new NullReferenceException($"FileGuid:{fileGuid}, IdentityNumber: {request.TcKimlikNo.Trim()} bilgilerine ait osya bilgisi bulunamadı."), "Talep etmiş olduğunuz dosya bilgisine ulaşılamadı. Lütfen bilgilerinizi kontrol ediniz.");
                ...
                if (File.Exists(fullFilePath))
                {
                    ...
                    var appealFileDownloadResponseModel = new BasvuruDosyaIndirCommandResponseModel();
